		...
		for (AActor* Target : ActorsInLine)
		{
			...
			if (GeoASLib::IsTeamAttitudeAligned(Avatar, Target, TeamAttitudeMask::Hostile))
			{
				...
				if (bIsServer)
				{
					...
					DamageEffect.Amount = FScalableFloat(BaseDamage.GetValueAtLevel(GetAbilityLevel()) * Multiplier);
			...
			else if (GeoASLib::IsTeamAttitudeAligned(Avatar, Target, TeamAttitudeMask::FriendlyOrNeutral))
			{
				...
				if (bIsServer)
				{
					...
					ShieldEffect.Amount = FScalableFloat(BaseShield.GetValueAtLevel(GetAbilityLevel()) * Multiplier);
