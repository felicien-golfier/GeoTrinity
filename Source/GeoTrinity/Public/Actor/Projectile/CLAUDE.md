...
**`BulletVFX` + params:** `BulletVFX` is a **native** `UNiagaraComponent` subobject; its system asset (`NS_GeoTrinity_Projectile01`) is set in the C++ constructor (BP may override the asset field — but **not** its Niagara User Params: those never populate on an inherited component, which is why radius/colors are C++-driven, not asset-driven). The component is **write-only** from C++ — nothing reads its user params back. The per-Blueprint default values live in `FProjectileParams DefaultParams` (edited in Class Defaults; `Radius` is gameplay — it sizes the collider — plus head/trail color + trail lifetime). `ApplyParams(FProjectileParams)` is the single write path: it resizes the `Sphere` collider and pushes the four user params `User.Bullet_Radius`/`Bullet_HeadColor`/`Bullet_TrailColor`/`Trail_LifetimeScale`. `ApplyProjectileParams(FGeoProjectileParams)` resolves each value via its `EOverrideParam` (settings / `DefaultParams` / explicit override) into a resolved `FProjectileParams` and calls `ApplyParams`. `FGeoProjectileParams` **derives** from `FProjectileParams` (values shared, one declaration) and adds the per-value toggles + distance/speed; inherited value fields can't carry `EditConditionHides` so they always show. Simulated proxies (which never run `ApplyProjectileParams`) apply `DefaultParams` in `BeginPlay`. Editor-only `PreviewParams()` (`CallInEditor` button) pushes `DefaultParams` + `ReinitializeSystem()` so the result is visible without PIE — click it on a placed instance / BP preview actor, not Class Defaults. `AGeoShieldBurstProjectile` drives `BulletVFX` directly for its per-bounce radius (always valid now that it's a native subobject).
