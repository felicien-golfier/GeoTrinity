...
- `BuildCombattantLifeBarWidget(Blueprint, BarWidth, BarHeight)` — builds WBP_CombattantLifeBar tree: SizeBox root → Overlay → HealthBar under ShieldBar (semi-transparent cyan, shield-over-health like WBP_MainOverlay). Names match BindWidgetOptional members on `UGenericCombattantWidget`. Reuses the existing SizeBox dimensions if the asset already has them
