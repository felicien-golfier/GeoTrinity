...
- **Character N is local player N, and it holds platform user N.** That identity is the whole trick and it is self-sustaining: player 0 is created by the engine on the primary user (id 0), and every gamepad joins with the id `ApplyCouchCoopSetting()` handed it. So "put this gamepad on character N" is just "give it platform user N" — no bookkeeping, no `AllocateNewUserId`.
...
- **`ApplyCouchCoopSetting()` is the whole rule.** It seats the connected gamepads (device-id order, keyboard excluded) on characters starting at `FirstCharacter`, which `UGeoGameUserSettings::UseFirstGamepadForSecondPlayer` (default off) moves from 0 to 1:
  - **Off** — gamepad 1 lands on character 1 and shares it with the keyboard; gamepad 2 lands on character 2, which owns nobody yet, and creates it on **Start** (`EKeys::Gamepad_Special_Right`).
  - **On** — everyone starts one further along: gamepad 1 takes character 2 (immediately, if it already exists), gamepad 2 takes character 3.
  - **A gamepad already playing keeps its character**, so a controller that disconnects and comes back drops into the character its old device left behind — reconnecting hands Windows a *new* device id, and without this the id order would re-seat the whole row and shuffle everyone. The seated pads are pulled out of the list first; whatever is left fills the lowest free characters in device order.
  - **Flipping the setting is the one thing that re-seats everyone** — detected by `AppliedFirstCharacter` changing, since every gamepad has to shift a character up or down. That is deliberately the opposite of the reconnect rule.
  - Finally, any local player above 0 whose platform user has **no mapped device left** is removed: that is the shift dropping a character nobody drives. A *disconnected* pad still counts as mapped, so unplugging a controller never deletes its character — there has to be one left for it to take back.
- It is called from `Init`, from `UGeoKeyBindingsWidget` on toggle, and **from `InputKey` whenever a gamepad key arrives with no owning local player**. That last call is not a fallback: a pad plugged in after launch gets a fresh platform user from the mapper, and re-deriving its owner there — before `Super::InputKey` routes it — is what stops its first press being dropped. It is also why reconnecting takes effect on the first stick nudge rather than needing Start.
