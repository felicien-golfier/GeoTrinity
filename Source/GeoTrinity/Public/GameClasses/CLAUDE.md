...
- **`ApplyCouchCoopSetting()` is the whole rule**, and it reads `UGeoGameUserSettings::UseFirstGamepadForSecondPlayer` (default off):
  ...
  - **On** — one gamepad holds `SecondPlayerUser` (allocated once via `AllocateNewUserId`, reused across toggles so flipping cannot exhaust the 8 platform users) and joins as player 2 on **Start** (`EKeys::Gamepad_Special_Right`). Every **other** gamepad still collapses onto the keyboard's user — that is what makes "2 players, 2 pads" work: pad A is player 2, pad B drives player 1 alongside the idle keyboard.
  - **The holder is sticky, and that matters.** The lowest device id claims `SecondPlayerUser` only when *no connected pad already holds it*. Re-deriving "first gamepad" from ids on every call is a live bug: plugging in a second pad re-runs the assignment, and any pad enumerating below the current holder would steal the user, dropping player 2's pad back onto player 1 — both pads then drive one character while the checkbox still reads on. A holder that disconnects releases the user, so the next pad in takes over player 2.
