...
**Damage redirection** — `UGeoAttributeSetBase::PostGameplayEffectExecute` calls the static `TryRedirectIncomingDamage(VictimASC, DamageContext, Damage)` before shield absorption: if the victim carries `Status.Sacrificed` (and the damage isn't excluded via the `bDoNotRedirectSacrifice` context flag), the victim takes **zero** damage; the mark GE's context leads back to the Square's ASC → active channel instance (via `Spec.GetAbilityInstances()` — `GetPrimaryInstance` is null for `InstancedPerExecution`) → `RedirectCapturedDamage`: the amount is added to `SacrificeValue` and split equally across alive `AGeoWall`s (recomputed per event, re-validated at apply time, Square damaged last) + the Square, applied as normal damage with `bDoNotRedirectSacrifice=true`. Walls themselves can be sacrificed; the deployable life drain sets `bDoNotRedirectSacrifice` so it is never captured.
