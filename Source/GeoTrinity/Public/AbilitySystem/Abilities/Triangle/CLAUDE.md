...
- `GetColorForAmmo(int32 Ammo)` — `BlueprintPure`; maps ammo count to a color the same way `Fire()` maps it to a buff index, so the HUD ammo number can preview the color before reload.
