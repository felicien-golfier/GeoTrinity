...
Extends `UPatternAbility`, launches `UTileBombPattern`. `CreatePatternData()` draws one live player (`FRandomStream(StoredPayload.Seed)` via `RandHelper`, never `Seed % Num` which goes negative) and ships it as `FTileBombPatternData::BombCarrier` — resolved once on the server so every machine marks the same player. Ships an empty carrier (pattern fizzles) if nobody's alive.
