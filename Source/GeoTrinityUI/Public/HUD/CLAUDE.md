...
| `GeoOverlayWidget.h` | Root player overlay; `AbilityBar`/`StatusBar` as BindWidgets, driven from C++. `ApplyColumnLayout()` squeezes the root canvas children into this player's column — every local player's overlay lands in the **same** viewport, so the authored solo layout would stack them. Columns are `1/NumLocalPlayers` of the width: solo is the whole screen (identity, exactly as authored), two players get halves, three get **left / middle / right** by local player index. It maps anchor X by `(x + Column) / ColumnCount`, first undoing the previously applied column (`AppliedColumn`/`AppliedColumnCount`) so a third player joining re-widths everyone instead of compounding — which is why it re-runs on both `OnLocalPlayerAddedEvent` and `OnLocalPlayerRemovedEvent`, not just once. Layout is C++, not BP, because the choice is runtime-only — the designer has one layout and the player index isn't known until construction |
