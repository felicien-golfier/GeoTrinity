...
| `Menu/GeoSettingsWidget.h` | Settings sub-panel (Sound + Key Bindings starting point); BindWidgets: BackButton (UGeoMenuButton), MasterVolumeSlider (USlider, placeholder — no audio mixer wired up yet), KeyBindingsList (UScrollBox, read-only rows built from `UAbilityInfo::GetAllPlayersAbilityInfos()`, no remapping); `OnClosed` fires on Back |
