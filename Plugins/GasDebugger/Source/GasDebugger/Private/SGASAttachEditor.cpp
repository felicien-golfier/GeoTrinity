...
FText GetOverrideTypeDropDownText_Explicit(const TWeakObjectPtr<UAbilitySystemComponent>& InComp)
{
	...
	FString NameToDisplay = "Not loaded yet";
	if (LocalAvatarActor)
	{
		NameToDisplay = LocalOwnerActor->GetName();
	}
	else if (LocalAvatarActor)
	{
		NameToDisplay = LocalAvatarActor->GetName();
	}
	...
	FText OutName = FText::FromString(NameToDisplay);
