...
def probe_switch_entries(system_path, emitter_name, usage, function_name, switch_name, candidates):
    """Returns the candidate display names a static switch accepts; the setter matches by display name.
    Leaves the switch on the last accepted candidate, so write the wanted entry after probing."""
    cdo = builder()
    return [entry for entry in candidates
            if cdo.set_static_switch(system_path, emitter_name, usage, function_name, switch_name, entry)]
...
def example_probe_a_distribution_switch():
    """Reports which entries one switch accepts, since a user-defined enum lists none of them to Python.

    Deliberately not invoked — probing writes to the switch, so it changes the branch the stack is on.
    """
    system = "/Game/Art/VFX/Assets/NS_DeathDebrisTriangle.NS_DeathDebrisTriangle"
    accepted = probe_switch_entries(
