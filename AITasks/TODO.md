...
<!-- [2026-04-15] Processed new Square ability headers from the Apr 13 commit. Added JavaDoc to: UGeoGameplayAbility::OnAvatarSet (auto-activates passives on avatar set — non-obvious trigger); AGeoMine public API (InitInteractableData, GetLifetimeReplicatedProps, Recall — Recall in particular warrants documentation because it doubles as both the step-on detonation path and the explicit recall path, driven by the same sphere-overlap logic with Value scaling); FShieldEffectData struct-level comment. No malformed or contradictory comments found in the changed files. -->
