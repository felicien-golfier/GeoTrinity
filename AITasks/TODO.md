...
<!-- [2026-04-23] Processed 5 headers from the "Advance on the shield burst" commit (2026-04-22). Fixed malformed double-/** comment on InitProjectileLife (merged into one block with @warning server-only). Added docs to new GetAbilityCDO<T> template and non-template overload. Added docs to all 24 context getter/setter functions in GeoAbilitySystemLibrary.h (were undocumented); the section-header `/** Getters and setters for the context */` comment was misplaced as if it were a function doc for IsBlockedHit — replaced with a plain `//` section marker. Added struct-level docs for FGameplayAbilityInfo and FPlayersGameplayAbilityInfo; fixed empty `/** * */` class comment on UAbilityInfo. Added doc for new AreNativeTagsInitialized(). Converted inline C++ comment on ChargeTime to JavaDoc. No latent bugs observed. -->
