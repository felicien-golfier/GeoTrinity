...
<!-- [2026-04-14] Processed 8 headers from commits since 2026-04-10 (Square abilities + camera rework). EffectData.h was missing its copyright header entirely — pre-existing gap. Added class-level docs to: UEffectDataAsset, FGameplayEffectData, FDamageEffectData, FHealEffectData, FShieldEffectData, FContextDamageMultiplierEffectData, FStatusEffectData, AGeoProjectile (class-level was missing), FGeoGameplayEffectContext. Added function-level docs to: GeoGameplayAbility (OnAvatarSet, ActivateAbility, EndAbility override, InputReleased); GeoShieldBurstPassiveAbility::OnGaugeChanged delegate; GeoDeployableBase::Recall (expanded with Value param docs); GeoMine (InitInteractableData, GetLifetimeReplicatedProps, Recall override). Improved GameDataSettings class comment. Notable: CLAUDE.md refers to FContextDamageMultiplierEffectData as FSingleUseDamageMultiplierEffectData — the names are out of sync and could cause confusion when following the architecture docs. -->
