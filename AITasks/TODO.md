...
<!-- [2026-04-18] Processed 18 headers from commits 2026-04-12 through 2026-04-16 (6 commits since last run). Added copyright headers to EffectData.h and GeoGameplayTags.h (both pre-existing omissions). Added class-level JavaDoc to: UEffectDataAsset, FGameplayEffectData, FDamageEffectData, FHealEffectData, FShieldEffectData, FContextDamageMultiplierEffectData, FStatusEffectData (all in EffectData.h), FGeoGameplayEffectContext (GeoAscTypes.h). Added function-level docs to AGeoMine::InitInteractableData and AGeoMine::Recall(float). Improved UGameDataSettings class comment (was informal one-liner). Most other modified headers were already well-documented from the previous run. No suspicious patterns or contradicting comments found. -->
