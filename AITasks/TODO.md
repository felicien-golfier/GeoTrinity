...
<!-- [2026-04-17] git diff --name-only HEAD was empty (no uncommitted changes). Scanned April 13–16 commits instead. Fixed 3 files: (1) EffectData.h — added missing copyright header and a class-level doc to UEffectDataAsset (pre-existing omission since file was first added). (2) GeoProjectile.h — added missing class-level doc (class had per-method docs but no class overview). (3) GeoAbilitySystemLibrary.h — added inline docs to GetAllAgentsInTeam and GetAllAgentsWithRelationTowardsActor. All other April 13–16 headers (Square/Circle abilities, GeoMine, GeoShieldBurstProjectile, GeoDeployableBase, GeoDeployAbility) were already adequately documented. -->
