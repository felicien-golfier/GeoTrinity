...
<!-- [2026-04-11] Second-pass sweep of 2 commits from April 10 (a1bc0a2, b3c3f88) that post-dated the previous run. No uncommitted changes today. Findings: 6 files were missing the copyright header (EffectData.h, GeoCharacter.h, GeoMovementComponent.h, UGeoGameplayLibrary.h, GeoPlayerState.h, GeoDeployableManagerComponent.h) — all added. Weak/placeholder class comments replaced: AGeoPlayerState ("Deriving just to set up basic RPG stuff for now"), UAbilityInfo (empty /** */), UGeoGameplayLibrary (no class comment), AGeoCharacter (no class comment), UGeoAssetManager, UGeoUserWidget, AGeoHUD. GeoCombatStatsSubsystem was entirely undocumented — class, two internal structs, and all 4+ public methods documented. EffectData.h subclass structs (FGameplayEffectData, FDamageEffectData, FHealEffectData, FContextDamageMultiplierEffectData, FStatusEffectData) had no class-level comments. Notable: FContextDamageMultiplierEffectData had no documentation explaining the scoped-per-call semantics which is a non-obvious invariant (multiplier auto-resets each ApplyEffectFromEffectData call). Fixed. -->
