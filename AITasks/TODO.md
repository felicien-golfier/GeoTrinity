...
<!-- [2026-04-16] Processed 4 headers from commits since 2026-04-10 (b6387e3..6b7a4d3). EffectData.h: missing copyright header (pre-existing, fixed); UEffectDataAsset had no class doc; all 6 FEffectData subclasses (FGameplayEffectData, FDamageEffectData, FHealEffectData, FShieldEffectData, FContextDamageMultiplierEffectData, FStatusEffectData) had no class doc — notable since FShieldEffectData is brand-new in 6b7a4d3. FGeoGameplayEffectContext in GeoAscTypes.h had a stale file-level comment that was not a proper struct doc. GeoGameplayTags.h: missing copyright header (pre-existing). GeoMine.h: new file from 6b7a4d3 — public Recall and InitInteractableData had no docs. Nothing suspicious beyond the two missing copyright headers, which are pre-existing omissions in files that predate the copyright enforcement policy. -->
